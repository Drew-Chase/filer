...
pub async fn get_all() -> anyhow::Result<std::collections::HashMap<String, String>> {
	let pool = create_pool().await?;
	let rows: Vec<(String, String)> = sqlx::query_as("SELECT key, value FROM internal_configuration")
		.fetch_all(&pool)
		.await?;
...
pub async fn get(key: &str) -> anyhow::Result<Option<String>> {
	let pool = create_pool().await?;
	let row: Option<(String,)> = sqlx::query_as("SELECT value FROM internal_configuration WHERE key = ?")
		.bind(key)
		.fetch_optional(&pool)
		.await?;
...
pub async fn set_all(
	...
	for (key, value) in map {
		items.push(sqlx::query("INSERT INTO internal_configuration (key, value) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value")
			.bind(key.clone())
			.bind(value.clone())
			.execute(&pool));
	}
