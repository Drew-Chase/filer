    ...
    fn call(&self, req: ServiceRequest) -> Self::Future {
        ...
        if !config.authorized_hosts.is_empty() {
            ...
            let is_authorized = config.authorized_hosts.iter().any(|host| {
                ...
                if let (Ok(host_ip), Ok(client_addr)) = (IpAddr::from_str(host), IpAddr::from_str(client_ip)) {
        ...
        Box::pin(async move {
            ...
            if config.cors_enabled {
                ...
                res.headers_mut().insert(header::ACCESS_CONTROL_ALLOW_METHODS, header::HeaderValue::from_static("GET, POST, PUT, DELETE, OPTIONS"));
                res.headers_mut()
                    .insert(header::ACCESS_CONTROL_ALLOW_HEADERS, header::HeaderValue::from_static("Content-Type, Authorization, Accept"));
